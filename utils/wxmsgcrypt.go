...
//NewWXBizMsg4Send 组装WXBizMsg4Send数据
...
//NewWXBizMsgCrypt 获取WXBizMsgCrypt实例
func NewWXBizMsgCrypt(protocol_type ProtocolType) *WXBizMsgCrypt {
	...
	return &WXBizMsgCrypt{protocol_processor: protocol_processor}
}
...
func (self *WXBizMsgCrypt) cbcEncrypter(aesKey, plaintext string) ([]byte, *CryptError) {
...
func (self *WXBizMsgCrypt) cbcDecrypter(aesKey, base64_encrypt_msg string) ([]byte, *CryptError) {
	encoding_aeskey := aesKey + "="
	aeskey, err := base64.StdEncoding.DecodeString(encoding_aeskey)
	if nil != err {
		return nil, NewCryptError(DecodeBase64Error, err.Error())
...
func (self *WXBizMsgCrypt) calSignature(token, timestamp, nonce, data string) string {
	sort_arr := []string{token, timestamp, nonce, data}
...
//VerifyURL url认证
func (self *WXBizMsgCrypt) VerifyURL(corpID, token, aesKey, msg_signature,
	timestamp, nonce, echostr string) ([]byte, *CryptError) {
	signature := self.calSignature(token, timestamp, nonce, echostr)

	if strings.Compare(signature, msg_signature) != 0 {
		return nil, NewCryptError(ValidateSignatureError, "signature not equal")
	}

	plaintext, err := self.cbcDecrypter(aesKey, echostr)
	if nil != err {
		return nil, err
	}
	...
	if len(corpID) > 0 && strings.Compare(string(receiver_id), corpID) != 0 {
...
//EncryptMsg 加密数据
func (self *WXBizMsgCrypt) EncryptMsg(corpID, token, aesKey, reply_msg,
	timestamp, nonce string) ([]byte, *CryptError) {
	...
	buffer.WriteString(corpID)

	tmp_ciphertext, err := self.cbcEncrypter(aesKey, buffer.String())
	if nil != err {
		return nil, err
	}
	ciphertext := string(tmp_ciphertext)

	signature := self.calSignature(token, timestamp, nonce, ciphertext)
...
//EncryptMsg 解密数据
func (self *WXBizMsgCrypt) DecryptMsg(corpID, token, aesKey, msg_signature, timestamp, nonce string,
	post_data []byte) ([]byte, *CryptError) {
	msg4_recv, crypt_err := self.protocol_processor.parse(post_data)
	if nil != crypt_err {
		return nil, crypt_err
	}

	signature := self.calSignature(token, timestamp, nonce, msg4_recv.Encrypt)

	if strings.Compare(signature, msg_signature) != 0 {
		return nil, NewCryptError(ValidateSignatureError, "signature not equal")
	}

	plaintext, crypt_err := self.cbcDecrypter(aesKey, msg4_recv.Encrypt)
	if nil != crypt_err {
		return nil, crypt_err
	}
